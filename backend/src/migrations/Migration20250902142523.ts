...
export class Migration20250902142523 extends Migration {
  override async up(): Promise<void> {
    this.addSql(
      `create table "product" ("id" serial primary key, "name" varchar(255) not null, "description" text null, "price" numeric(10,2) not null, "stock" int not null default 0, "image_url" varchar(255) null, "category" varchar(255) not null, "created_at" timestamptz not null, "updated_at" timestamptz not null);`
    );

    this.addSql(
      `create table "user" ("id" serial primary key, "email" varchar(255) not null, "password" varchar(255) not null, "role" varchar(255) not null default 'customer', "created_at" timestamptz not null, "updated_at" timestamptz not null);`
    );
    this.addSql(`alter table "user" add constraint "user_email_unique" unique ("email");`);

    this.addSql(
      `create table "order" ("id" serial primary key, "user_id" int not null, "total_amount" numeric(10,2) not null, "status" varchar(255) not null default 'pending', "created_at" timestamptz not null, "updated_at" timestamptz not null);`
    );

    this.addSql(
      `create table "order_item" ("id" serial primary key, "order_id" int not null, "product_id" int not null, "quantity" int not null, "price" numeric(10,2) not null, "created_at" timestamptz not null);`
    );
